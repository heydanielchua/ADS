...
So applying the standard 60->50 rule to accent makes the brand color **blue in light mode, cyan in dark mode** - intended, not a bug. `semantic.brand.primary` therefore points at the accent ramp (not the blue tint ramp). Feedback and other roles use their normal `.60 -> .50` family steps.
