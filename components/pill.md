...
| Selected | `semantic.accent.primary` | same | `semantic.accent.onAccent` |
| Cancel Selected | `semantic.accent.primary` | same | `semantic.accent.onAccent` + cancel chip (`semantic.surface.pressed`) |
| Disabled | transparent | 1px `semantic.border.strong` | `semantic.text.default`, 40% opacity |

## Rules
- Always pill-shaped. Selected uses `semantic.accent.primary` (cyan in dark mode).
